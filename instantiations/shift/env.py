...
def default_shift_augs(
    ...
    def aug2(im: torch.Tensor) -> torch.Tensor:
        """Channel shuffle + color jitter augmentation (distinct from aug1)."""
        out = im.clone()
        if im.shape[1] >= 3:
            # Shuffle color channels to create a structurally different augmentation
        ...
        if greyscale_prob > 0 and torch.rand(1).item() < greyscale_prob:
            g = out.mean(dim=1, keepdim=True).expand_as(out)
            out = g  # Full greyscale conversion instead of partial blend
        return out.clamp(0, 1)
